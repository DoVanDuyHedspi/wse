<template>
  <div class="p-4 bg-white" id="on-leave-type">
    ...
    <div class="text-center mt-3">
      ...
      <el-button type="primary" @click="createLeaveType('form-leave-type')" size="medium">Thêm mới</el-button>
    </div>
  </div>
</template>
    ...
    createLeaveType(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          axios.post("/api/leave_type", this.form).then(response => {
            if (response.data.status === false) {
              ...
              this.$notify({
                title: "Hoàn thành",
                message: "Thêm mới loại ngày nghỉ phép thành công",
                type: "success",
                position: "bottom-right"
              });
              this.$router.push("/working-day/on-leave-type");
