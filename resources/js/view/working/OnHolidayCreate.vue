<template>
  <div class="p-4 bg-white" id="on-holiday-create">
    ...
    <div class="text-center mt-3">
      <el-button type="default" @click="cancel('form-holiday')" size="medium">Hủy</el-button>
      <el-button type="primary" @click="createHoliday('form-holiday')" size="medium">Thêm mới</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    createHoliday(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          axios.post("/api/holiday", this.form).then(response => {
